//EXTERNAL EXEC
//PlayerStartAction(1, 'man_geralt_sword_attack_fast_1_lp_40ms'); NEED REWORK TO FAST SWITCH
...
function _CalculateHealthEnemy(entity:CNewNPC, ClientSHealth:int)
{
    ...
    if (thePlayer.GetTarget().GetGuidHash() == entity.GetGuidHash() && thePlayer.IsInCombat()) // IF PLAYER 1, 2,.. OTHER ATTACK ONE OBJECT
        ((CActor)entity).DrainVitality((entity.GetCurrentHealth() / 100) * (ClientSHealth / entity.GetCurrentHealth())); //Percent of client healthenemy and Get value of damage
    else
        ((CActor)entity).SetHealthPerc(ClientSHealth); //If > one player target one object
}
function _SetCombatTarget(entity:CNewNPC, entity2:CNewNPC)
{
    ...
    else if (dist > 2 && dist < 4) _Teleport(entity, entity2.GetWorldPosition()); //TELEPORT SOME FORWARD FOR ATTACK ACTION
