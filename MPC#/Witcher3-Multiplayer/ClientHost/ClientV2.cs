            ...
            var a = Task.Run(() =>
            {
                while (IsConnected)
                {
                    ...
                    if (!CombatTargetsClient.ContainsKey(myCombat))
                    {
                        ...
                        foreach (var aa in CombatTargetsClient)//SEARCH FOR DEAD ENTITY AND SEND TO SERVER
                        {
                            var curta = aa.Value;
            ...
            if (data != null & data.Length >= 8)
            {
                ...
                switch (head)
                {
                    ...
                    case RecvSendTypes.RCV_PLAYERCOMBATTARGET:
                        ...
                        if (debug) LOG("[client] ClientID: " + IDClient + "  Recieve target: " + combatstruct.Guid + ":" + combatstruct.Template + " :dead?: " + combatstruct.IsDead + " AttackType: " + combatstruct.FightState);
