...
namespace Witcher3_Multiplayer.ClientHost
{
    public class ServerV2
    {
        ...
        {
            if (!SocketV2.IsConneted() & !Dedicated)
            {
            ...
            if (!Dedicated & GameManagerUI.IsInMenu() & GameManagerUI.IsGameStopped() & GameManagerUI.IsGamePaused())
            {
            ...
            if (data != null & data.Length >= 4)
            {
                ...
                switch (head)
                {
                    ...
                    case RecvSendTypes.SND_PLAYERCOMBATTARGET:
                        if (PlayerDataServer.ContainsKey(fromclie))
                        {
                            if (debug) LOG("[host] PlayerID " + IDClient + " receive combat target: " + recvdata.ToStructure<CombatTarget>().Guid);
